...
describe('importEmployeesAction', () => {
  beforeEach(() => {
    mockEmployee.findMany.mockResolvedValue([]); // no existing emails
  });

  it('returns FORBIDDEN for VIEWER', async () => {
    mockAuth.mockResolvedValue(makeSession('VIEWER'));
    const result = await importEmployeesAction([
      { fullName: 'Ana', email: 'ana@test.com', phone: null, position: null, department: null, city: null, location: null, isActive: null },
    ]);
    expect(result.ok).toBe(false);
    if (!result.ok) expect(result.code).toBe('FORBIDDEN');
  });

  it('skips row with missing fullName and reports error', async () => {
    mockAuth.mockResolvedValue(makeSession('ADMIN'));
    mockTransaction.mockResolvedValue({});
    const result = await importEmployeesAction([
      { fullName: null, email: 'ana@test.com', phone: null, position: null, department: null, city: null, location: null, isActive: null },
    ]);
    expect(result.ok).toBe(true);
    if (result.ok) {
      expect(result.data.inserted).toBe(0);
      expect(result.data.skipped).toBe(1);
      expect(result.data.errors[0].field).toBe('fullName');
    }
  });

  it('skips row with duplicate email within the file', async () => {
    mockAuth.mockResolvedValue(makeSession('ADMIN'));
    mockTransaction.mockResolvedValue({});
    const rows = [
      { fullName: 'Ana', email: 'dup@test.com', phone: null, position: null, department: null, city: null, location: null, isActive: null },
      { fullName: 'Ana2', email: 'dup@test.com', phone: null, position: null, department: null, city: null, location: null, isActive: null },
    ];
    ...
    if (result.ok) {
      ...
      expect(result.data.skipped).toBe(1);
      expect(result.data.errors[0].message).toMatch(/archivo/i);
    }
  });

  it('skips row with email already in DB', async () => {
    mockAuth.mockResolvedValue(makeSession('ADMIN'));
    mockEmployee.findMany.mockResolvedValue([{ email: 'existing@test.com' }]);
    const result = await importEmployeesAction([
      { fullName: 'Ana', email: 'existing@test.com', phone: null, position: null, department: null, city: null, location: null, isActive: null },
    ]);
    expect(result.ok).toBe(true);
    if (result.ok) {
      expect(result.data.skipped).toBe(1);
      expect(result.data.errors[0].message).toMatch(/base/i);
    }
  });

  it('reports row error when city name not found', async () => {
    mockAuth.mockResolvedValue(makeSession('ADMIN'));
    mockTransaction.mockImplementation(async (fn: (tx: typeof prisma) => Promise<unknown>) => {
      const tx = {
    ...
    const result = await importEmployeesAction([
      { fullName: 'Ana', email: 'ana@test.com', phone: null, position: null, department: null, city: 'CiudadFalsa', location: null, isActive: null },
    ]);
    expect(result.ok).toBe(true);
    if (result.ok) {
      expect(result.data.skipped).toBe(1);
      expect(result.data.errors[0].field).toBe('city');
    }
  });

  it('auto-upserts new department and inserts employee', async () => {
    mockAuth.mockResolvedValue(makeSession('ADMIN'));
    mockTransaction.mockImplementation(async (fn: (tx: typeof prisma) => Promise<unknown>) => {
      const tx = {
        department: { upsert: vi.fn().mockResolvedValue({ id: 'dept-new' }) },
    ...
    const result = await importEmployeesAction([
      { fullName: 'Ana', email: 'ana@test.com', phone: null, position: null, department: 'TI Nueva', city: null, location: null, isActive: null },
    ]);
    expect(result.ok).toBe(true);
    if (result.ok) expect(result.data.inserted).toBe(1);
  });

  it('coerces isActive "no" to false', async () => {
    mockAuth.mockResolvedValue(makeSession('ADMIN'));
    ...
    mockTransaction.mockImplementation(async (fn: (tx: typeof prisma) => Promise<unknown>) => {
      const tx = {
        ...
        location: { findFirst: vi.fn() },
        employee: {
          create: vi.fn().mockImplementation(({ data }: { data: { isActive: boolean } }) => {
            capturedIsActive = data.isActive;
    ...
    await importEmployeesAction([
      { fullName: 'Ana', email: 'ana@test.com', phone: null, position: null, department: null, city: null, location: null, isActive: 'no' },
    ]);
    expect(capturedIsActive).toBe(false);
  });

  it('processes mixed batch: 2 ok, 1 error (duplicate email)', async () => {
    mockAuth.mockResolvedValue(makeSession('ADMIN'));
    mockEmployee.findMany.mockResolvedValue([{ email: 'dup@test.com' }]);
    mockTransaction.mockImplementation(async (fn: (tx: typeof prisma) => Promise<unknown>) => {
      const tx = {
        department: { upsert: vi.fn() },
        city: { findFirst: vi.fn() },
    ...
    const rows = [
      { fullName: 'A', email: 'a@test.com', phone: null, position: null, department: null, city: null, location: null, isActive: null },
      { fullName: 'B', email: 'dup@test.com', phone: null, position: null, department: null, city: null, location: null, isActive: null },
      { fullName: 'C', email: 'c@test.com', phone: null, position: null, department: null, city: null, location: null, isActive: null },
    ];
    const result = await importEmployeesAction(rows);
    expect(result.ok).toBe(true);
    if (result.ok) {
